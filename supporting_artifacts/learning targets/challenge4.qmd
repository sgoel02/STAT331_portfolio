...
houses <- houses |> 
  ...
  group_by(region) |> 
  summarize(house_price = mean(house_prices), .groups = "keep")
```
...
avo_new <- avocados |> 
  ...
  group_by(region) |> 
  summarize(total_sales = mean(`Total Volume`)) |> 
...
```{r region-sales-plot}
ggplot(data = avo_new, mapping = aes(x = region, y = total_sales)) +
  geom_col() +
  xlab("Region of CA") +
  ylab("Average Total Avocado Sales")
```

```{r region-houseprice-plot}
ggplot(data = avo_new, mapping = aes(x = region, y = house_price)) +
  geom_col() +
  xlab("Region") +
